...
export async function initNotesView() {
        ...
        try {
            ...
            notesList.querySelectorAll('[data-action="delete-note"]').forEach(button => {
                button.addEventListener('click', async (event) => {
                    ...
                    if (confirm('Weet u zeker dat u deze notitie wilt verwijderen?')) { // Voor nu nog confirm
                        try {
    ...
    if (noteForm) {
        noteForm.addEventListener('submit', async (event) => {
            ...
            const note = {
                id: noteIdInput.value ? parseInt(noteIdInput.value) : undefined, // Gebruik undefined voor autoIncrement
    ...
    await loadNotes(); // Laad notities bij initialisatie van de view
}
